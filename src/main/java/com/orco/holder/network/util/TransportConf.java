...
package com.orco.holder.network.util;
...
public class TransportConf {
  ...
  public int connectionTimeoutMs() {
    long defaultNetworkTimeoutS = JavaUtils.timeStringAsSec(
      conf.get("holder.network.timeout", "120s"));
  ...
  public int authRTTimeoutMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get("holder.network.auth.rpcTimeout",
  ...
  public int memoryMapBytes() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("holder.storage.memoryMapThreshold", "2m")));
  }
  ...
  public int portMaxRetries() {
    return conf.getInt("holder.port.maxRetries", 16);
  }
  ...
  public boolean encryptionEnabled() {
    return conf.getBoolean("holder.network.crypto.enabled", false);
  }
  ...
  public String cipherTransformation() {
    return conf.get("holder.network.crypto.cipher", "AES/CTR/NoPadding");
  }
  ...
  public String keyFactoryAlgorithm() {
    return conf.get("holder.network.crypto.keyFactoryAlgorithm", "PBKDF2WithHmacSHA1");
  }
  ...
  public int keyFactoryIterations() {
    return conf.getInt("holder.networy.crypto.keyFactoryIterations", 1024);
  }
  ...
  public int encryptionKeyLength() {
    return conf.getInt("holder.network.crypto.keyLength", 128);
  }
   ...
   */
  public int ivLength() {
    return conf.getInt("holder.network.crypto.ivLength", 16);
  }
  ...
  public String keyAlgorithm() {
    return conf.get("holder.network.crypto.keyAlgorithm", "AES");
  }
  ...
  public boolean saslFallback() {
    return conf.getBoolean("holder.network.crypto.saslFallback", true);
  }
  ...
  public boolean saslEncryption() {
    return conf.getBoolean("holder.authenticate.enableSaslEncryption", false);
  }
  ...
  public int maxSaslEncryptedBlockSize() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("holder.network.sasl.maxEncryptedBlockSize", "64k")));
  }
  ...
  public boolean saslServerAlwaysEncrypt() {
    return conf.getBoolean("holder.network.sasl.serverAlwaysEncrypt", false);
  }
  ...
  public Properties cryptoConf() {
    return CryptoUtils.toCryptoConf("holder.network.crypto.config.", conf.getAll());
  }
   ...
   * retry according to the shuffle retry configs (see `holder.shuffle.io.maxRetries` and
   * `holder.shuffle.io.retryWait`), if those limits are reached the task will fail with fetch
   * failure.
   */
  public long maxChunksBeingTransferred() {
    return conf.getLong("holder.shuffle.maxChunksBeingTransferred", Long.MAX_VALUE);
  }
}

...
import static com.orco.holder.launcher.CommandBuilderUtils.*;
...
//		} else
		if ((envJavaHome = System.getenv("JAVA_HOME")) != null) {
	...
	private void addOptionString(List<String> cmd, String options) {
		if (!isEmpty(options)) {
...
//		String jarsDir = findJarsDir(getSparkHome(), getScalaVersion(), !isTesting && !isTestingSql);
		String jarsDir = findJarsDir(getSparkHome(), getScalaVersion(), !isTestingSql);
...
//			path = System.getProperty("holder.test.home");
//		}
...
//	}
	 ...
	 * user-specified properties file, or the holder-defaults.conf file under the Spark configuration
	 * directory.
	 */

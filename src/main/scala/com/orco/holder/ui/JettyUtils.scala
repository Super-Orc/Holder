...
/**
  ...
  */
private[holder] object JettyUtils extends Logging {

  val SPARK_CONNECTOR_NAME = "holder"
  ...
  def createServlet[T <: AnyRef](
                                  ...
                                  conf: HolderConf): HttpServlet = {

    // holder-10589 avoid frame-related click-jacking vulnerability, using X-Frame-Options
    ...
    // Example: holder.ui.allowFramingFrom = https://example.com/
    val allowFramingFrom = conf.getOption("holder.ui.allowFramingFrom")
    ...
    new HttpServlet {
      ...
      // holder-5983 ensure TRACE is not supported
...
//        // holder-21176: Use the Jetty logic to calculate the number of selector threads (#CPUs/2),
...
//  }
  ...
  def addFilters(handlers: Seq[ServletContextHandler], conf: HolderConf) {
    val filters: Array[String] = conf.get("holder.ui.filters", "").split(',').map(_.trim())
    filters.foreach {
      case filter: String =>
        if (!filter.isEmpty) {
          ...
          conf.get("holder." + filter + ".params", "").split(',').map(_.trim()).toSet.foreach {
            param: String =>
              if (!param.isEmpty) {
          ...
          val prefix = s"holder.$filter.param."
          conf.getAll
            .filter { case (k, _) => k.length() > prefix.length() && k.startsWith(prefix) }
  ...
  def startJettyServer(
    ...
    try {
      ...
      // least be the number of acceptors and selectors plus 1. (See holder-13776)
      var minThreads = 1
      ...
      ServerInfo(server, httpPort, collection)
    } catch {
      case e: Exception =>
        server.stop()
...
private[holder] case class ServerInfo(
                                      server: Server,
                                      boundPort: Int,

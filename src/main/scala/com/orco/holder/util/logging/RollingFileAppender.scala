...
/**
  ...
  * @param conf                    HolderConf that is used to pass on extra configurations
  * @param bufferSize              Optional buffer size. Used mainly for testing.
  */
private[holder] class RollingFileAppender(
...
/**
  * Companion object to [org.apache.holder.util.logging.RollingFileAppender. Defines
  * names of configurations that configure rolling file appenders.
  */
private[holder] object RollingFileAppender {
  val STRATEGY_PROPERTY = "holder.executor.logs.rolling.strategy"
  val STRATEGY_DEFAULT = ""
  val INTERVAL_PROPERTY = "holder.executor.logs.rolling.time.interval"
  val INTERVAL_DEFAULT = "daily"
  val SIZE_PROPERTY = "holder.executor.logs.rolling.maxSize"
  val SIZE_DEFAULT = (1024 * 1024).toString
  val RETAINED_FILES_PROPERTY = "holder.executor.logs.rolling.maxRetainedFiles"
  val DEFAULT_BUFFER_SIZE = 8192
  val ENABLE_COMPRESSION = "holder.executor.logs.rolling.enableCompression"

...
import java.util.concurrent.{ConcurrentHashMap, ScheduledExecutorService, TimeUnit, TimeoutException}
import javax.annotation.Nullable

import com.orco.holder.HolderConf
import com.orco.holder.internal.Logging
import com.orco.holder.network.TransportContext
import com.orco.holder.network.client._
import com.orco.holder.network.netty.SparkTransportConf
import com.orco.holder.network.server._
import com.orco.holder.rpc._
import com.orco.holder.serializer.{JavaSerializer, JavaSerializerInstance, SerializationStream}
import com.orco.holder.util.{ByteBufferInputStream, ByteBufferOutputStream, ThreadUtils, Utils}
...
private[netty] class NettyRpcEnv(
  ...
  private[netty] val transportConf = SparkTransportConf.fromSparkConf(
    conf.clone.set("holder.rpc.io.numConnectionsPerPeer", "1"),
    "rpc",
    conf.getInt("holder.rpc.io.threads", 0))
  ...
  private[netty] val clientConnectionExecutor = ThreadUtils.newDaemonCachedThreadPool(
    ...
    conf.getInt("holder.rpc.connect.threads", 64))
  ...
  /**
    * A map for [[RpcAddress]] and [[Outbox]]. When we are connecting to a remote [[com.orco.holder.rpc.RpcAddress]],
  ...
  private[netty] def ask[T: ClassTag](message: RequestMessage, timeout: RpcTimeout): Future[T] = {
    ...
    val remoteAddr: RpcAddress = message.receiver.address
    ...
    try {
      ...
      val timeoutCancelable = timeoutScheduler.schedule(new Runnable {
        override def run(): Unit = {
          onFailure(new TimeoutException(s"$message,Cannot receive any reply from ${remoteAddr} " +
  ...
  //      if (fileDownloadFactory == null) {
  //        val module = "files"
  //        val prefix = "holder.rpc.io."
  //        val clone = conf.clone()
  //
  //        // Copy any RPC configuration that is not overridden in the holder.files namespace.
  //        conf.getAll.foreach { case (key, value) =>
  //          if (key.startsWith(prefix)) {
  //            val opt = key.substring(prefix.length())
  //            clone.setIfMissing(s"holder.$module.io.$opt", value)
  //          }
  //        }
  //
  //        val ioThreads = clone.getInt("holder.files.io.threads", 1)
...
/**
  ...
  *
  * @param conf            holder configuration.
...
private[netty] class NettyRpcEndpointRef(
                                          @transient private val conf: HolderConf,

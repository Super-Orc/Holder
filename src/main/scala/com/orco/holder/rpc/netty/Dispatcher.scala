...
import com.orco.holder.rpc.{RpcEndpoint, RpcEndpointAddress, RpcEndpointRef, RpcEnvStoppedException}
import com.orco.holder.util.ThreadUtils
...
private[netty] class Dispatcher(nettyEnv: NettyRpcEnv, numUsableCores: Int) extends Logging {
  ...
  private def postMessage(
    ...
    val error = synchronized {
      ...
      if (stopped) {
        ...
        Some(new HolderException(s"Could not find $endpointName."))
      } else {
  ...
  private val threadpool: ThreadPoolExecutor = {
    ...
    val numThreads = nettyEnv.conf.getInt("holder.rpc.netty.dispatcher.numThreads",

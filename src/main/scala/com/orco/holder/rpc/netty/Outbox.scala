...
import com.orco.holder.network.client.{RpcResponseCallback, TransportClient}
import com.orco.holder.rpc.{RpcAddress, RpcEnvStoppedException}
...
private[netty] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  /**
    ...
    * [[Outbox]] is stopped, the sender will be notified with a [[HolderException]].
    */
  def send(message: OutboxMessage): Unit = {
    ...
    if (dropped) {
      message.onFailure(new HolderException("Message is dropped because Outbox is stopped"))
    } else {
      drainOutbox()
    }
  }

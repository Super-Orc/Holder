...
/**
  ...
  */
private[holder] case class RpcEndpointAddress(rpcAddress: RpcAddress, name: String) {
  ...
  override val toString: String = if (rpcAddress != null) {
    s"holder://$name@${rpcAddress.host}:${rpcAddress.port}"
  } else {
    s"holder-client://$name"
  }
}

private[holder] object RpcEndpointAddress {
  ...
  def apply(sparkUrl: String): RpcEndpointAddress = {
    try {
      ...
      if (uri.getScheme != "holder" ||
        ...
        uri.getQuery != null) {
        throw new HolderException("Invalid holder URL: " + sparkUrl)
      }
      new RpcEndpointAddress(host, port, name)
    } catch {
      case e: java.net.URISyntaxException =>
        throw new HolderException("Invalid holder URL: " + sparkUrl, e)
    }
  }
}
